...
MP.transfer = {}
local MPT = MP.transfer
local MPG = MP.gui

function MPT.Init()
	MPT.name = MP.name .. "Transfer"
	MPT.CreateTransferDialog()
end

function MPT.Export( zone, pageId, index )
...
function MPT.Import( jsonText, zone, pageId, index )
	...
	if missing then MP.Log( GetString( MP_MSG_IMPORTGEARENOENT ), MP.LOGTYPES.ERROR ) else MP.Log( GetString(
		MP_MSG_IMPORTSUCCESS ) ) end
	...
	-- food
	foodIndex = MP.FindFood( { importTable.food } )
	if foodIndex then
...
function MPT.SearchItem( equipType, setId, prefTraitType, filter )
	...
	if IsBankOpen() and not MP.DISABLEDBAGS[ bankBag ] then
...
function MPT.ShowExportDialog( zone, pageId, index )
	local text = MPT.Export( zone, pageId, index )
	MPG.SetTooltip( MPT.helpButton, TOP, GetString( MP_EXPORT_HELP ) )
	MPT.title:SetText( GetString( MP_EXPORT ):upper() )
	MPT.editBox:SetText( tostring( text or "" ) )
	MPT.importButton:SetHidden( true )
	MPT.dialogWindow:SetHidden( false )
	SCENE_MANAGER:SetInUIMode( true, false )
	MPT.editBox:TakeFocus()
	MPT.editBox:SelectAll()
end

function MPT.ShowImportDialog( zone, pageId, index )
	MPG.SetTooltip( MPT.helpButton, TOP, GetString( MP_IMPORT_HELP ) )
	MPT.title:SetText( GetString( MP_IMPORT ):upper() )
	MPT.editBox:Clear()
	MPT.importButton:SetHidden( false )
	MPT.dialogWindow:SetHidden( false )
	SCENE_MANAGER:SetInUIMode( true, false )
	MPT.editBox:TakeFocus()
	MPT.importButton:SetHandler( "OnClicked", function( self )
		MPT.dialogWindow:SetHidden( true )
		local text = MPT.editBox:GetText()
		MPT.Import( text, zone, pageId, index )
	end )
end

function MPT.CreateTransferDialog()
	local window = WINDOW_MANAGER:CreateTopLevelWindow( "MephistoTransfer" )
	MPT.dialogWindow = window
