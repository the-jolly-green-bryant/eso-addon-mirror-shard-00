...
local version=1
if DsR and DsR.UI and DsR.UI.version>=version then return end

DsR 	 = DsR or {}
DsR.UI 	 = DsR.UI or {}

local number=1
local base=DsR.name=="DsRGuildRoster"
...
-------------------------------------------------------------------------------------------------------------------------------------------------
function DsR.UI.TopLevelWindow(name, parent, dims, anchor, hidden)
	--Validate arguments
...
-------------------------------------------------------------------------------------------------------------------------------------------------
function DsR.UI.Control(name, parent, dims, anchor, hidden)
	--Validate arguments
	local point=parent
...
-------------------------------------------------------------------------------------------------------------------------------------------------
function DsR.UI.Backdrop(name, parent, dims, anchor, center, edge, tex, hidden)	
	--Validate arguments
	parent=(parent==nil) and GuiRoot or parent
	if (dims=="inherit" or #dims~=2) then dims={parent:GetWidth(), parent:GetHeight()} end
...
-------------------------------------------------------------------------------------------------------------------------------------------------
function DsR.UI.Label(name, parent, dims, anchor, font, color, align, text, hidden)
	--Validate arguments
	parent=(parent==nil) and GuiRoot or parent
	if (#anchor~=4 and #anchor~=5) then return end
...
-------------------------------------------------------------------------------------------------------------------------------------------------
function DsR.UI.Texture(name, parent, dims, anchor, tex, hidden, layers, coords)
	--Validate arguments
...
-------------------------------------------------------------------------------------------------------------------------------------------------
function DsR.UI.Button(name, parent, dims, anchor, state, font, align, normal, pressed, mouseover, hidden)
	--Validate arguments
	if (name==nil or name=="") then return end
	parent=(parent==nil) and GuiRoot or parent
	if (dims=="inherit" or #dims~=2) then dims={parent:GetWidth(), parent:GetHeight()} end
...
-------------------------------------------------------------------------------------------------------------------------------------------------
function DsR.UI.SimpleButton(name, parent, dims, anchor, tex, hidden, func, tooltip)
	...
	elseif type(tex)=="number" then
		...
		tex="/DsRGuildHall/misc/buttons.dds"
	end
...
-------------------------------------------------------------------------------------------------------------------------------------------------
function DsR.UI.SlideBox(name, parent, dims, anchor, value, func, tooltip, tex, hidden)
	--Validate arguments
	hidden=(hidden==nil) and false or hidden
	if parent==nil then return end
	if dims=="inherit" or #dims~=2 then dims={parent:GetWidth(), parent:GetHeight()} end
...
-------------------------------------------------------------------------------------------------------------------------------------------------
function DsR.UI.CheckBox(name, parent, dims, anchor, value, func, tooltip, hidden, tex)
	local coords,rotation
	...
	control:SetTexture(tex or "/DsRGuildHall/misc/buttons.dds")
	control:SetHidden(hidden)
...
-------------------------------------------------------------------------------------------------------------------------------------------------
function DsR.UI.Slider(name, parent, dims, anchor, hidden, func, MinMaxStep, textedit, funcUpdate)
	...
	local control=DsR.UI.Control(name, parent, {dims[1]-edit_w-5,dims[2]}, anchor, hidden)
...
--	slider:SetHandler("OnMouseExit", function() ZO_Options_OnMouseExit(control) end)
	...
	if textedit then
		...
		slidervalue:SetFont(DsR.UI.Font("standard",fs-2,true))
	end
...
-------------------------------------------------------------------------------------------------------------------------------------------------
function DsR.UI.ComboBox(name, parent, dims, anchor, array, val, fun, hidden, scroll)
	--Validate arguments
	if (name==nil or name=="") then return end
	parent=(parent==nil) and GuiRoot or parent
	if (dims=="inherit" or #dims~=2) then dims={parent:GetWidth(), parent:GetHeight()} end
	...
	control.m_comboBox:SetFont(DsR.UI.Font("standard",fs,false))
...
-------------------------------------------------------------------------------------------------------------------------------------------------
function DsR.UI.TextBox(name, parent, dims, anchor, chars, val, fun, hidden)
	--Validate arguments
	parent=(parent==nil) and GuiRoot or parent
	if (dims=="inherit" or #dims~=2) then dims={parent:GetWidth(), parent:GetHeight()} end
	if (#anchor~=4 and #anchor~=5) then return end
...
-------------------------------------------------------------------------------------------------------------------------------------------------
function DsR.UI.TextBoxMulti(name, parent, dims, anchor, chars, val, fun, hidden)
	--Validate arguments
	parent=(parent==nil) and GuiRoot or parent
	if (dims=="inherit" or #dims~=2) then dims={parent:GetWidth(), parent:GetHeight()} end
	if (#anchor~=4 and #anchor~=5) then return end
...
-------------------------------------------------------------------------------------------------------------------------------------------------
function DsR.UI.TextBoxMultiBig(name, parent, dims, anchor, chars, val, fun, hidden)
	--Validate arguments
	parent=(parent==nil) and GuiRoot or parent
	if (dims=="inherit" or #dims~=2) then dims={parent:GetWidth(), parent:GetHeight()} end
	if (#anchor~=4 and #anchor~=5) then return end
...
-------------------------------------------------------------------------------------------------------------------------------------------------
function DsR.UI.ColorPicker(name, parent, dims, anchor, val, fun, hidden)
	--Validate arguments
	if parent==nil then return end
	if (#anchor~=4 and #anchor~=5) then return end
	hidden=(hidden==nil) and false or hidden
...
--Functions
-------------------------------------------------------------------------------------------------------------------------------------------------
function DsR.UI.Font(font, size, shadow, outline)
...
-------------------------------------------------------------------------------------------------------------------------------------------------
function DsR.ColorString(r,g,b,a)
	a=a or 1
...
-------------------------------------------------------------------------------------------------------------------------------------------------
function DsR.CallLater(name,ms,func,opt1,opt2)
	if ms then
