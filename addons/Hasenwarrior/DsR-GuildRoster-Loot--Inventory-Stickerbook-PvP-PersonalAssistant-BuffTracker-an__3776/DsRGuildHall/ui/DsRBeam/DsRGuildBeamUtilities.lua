...
function DsRBeam.Random( minVal, maxVal )
...
function DsRBeam.ShadowFunction( tbl, funcName, shadowFunc )
...
function DsRBeam.CloneTable( obj )

	if type( obj ) ~= 'table' then return obj end

	local tbl = { }
	for k, v in pairs( obj ) do
		tbl[ DsRBeam.CloneTable( k ) ] = DsRBeam.CloneTable( v )
	end
...
function DsRBeam.IsListValue( list, value )
...
function DsRBeam.GetUnitPosition( unitTag )
